...
describe("SegmentSelectorEditorTest", function () {
    ...
    it('should not allow comparing segments more than the limit set', async function() {
        ...
        let title = await page.$eval(
            '.segmentationContainer .segmentList li:last-child',
        ...
        const liElemLength = await page.$eval(
            '.segmentListContainer .segmentList',
        ...
        let comparedCount = await page.$eval(
            '.segmentListContainer .segmentList',
            el => el.querySelectorAll('li.comparedSegment').length
        );
        ...
        for (let i = 0; i < liElemLength; i++) {
            ...
            const segmentHandle = await page.$(
                `.segmentListContainer .segmentList li:nth-child(${i + 1}) span.compareSegment`
            );

            await segmentHandle.click();
